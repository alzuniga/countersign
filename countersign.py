...
class Countersign(Gtk.Window):
	...
	def generate_btn_clicked_cb(self, widget):
		...
		self.check_options()
		#except GLib.GError, e:
		#   e_runtime = self.MessageDialog(flags=DIALOG_MODAL,type=gtk.MESSAGE_ERROR, buttons=gtk.BUTTONS_OK)
		#   e_runtime.set_markup("No password options selected")
		#   e_runtime.run()
		#   pass
	...
	def check_options(self):
		...
		try:
			...
			e_runtime = Gtk.MessageDialog(self, 0, Gtk.MessageType.ERROR,
						Gtk.ButtonsType.OK, "Runtime Error")
			e_runtime.format_secondary_text("Gtk.Builder error({0}): {1}".format(e.code, e.message))
			e_runtime.run() 
			e_runtime.destroy()
